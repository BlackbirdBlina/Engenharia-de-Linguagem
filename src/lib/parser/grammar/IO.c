...
void check_type_prefix(str prefix, ID_t $1) {
    ...
    if (prefix == NULL) {
        printf("ERROR Line %d: \"%s\" has type '%s', which is not printable\n",
               yylineno, $1, getVarType($1));
        exit(1);
    }
}
...
void VALUE_STRING_toPrint(Record** $$, VALUE_STRING_t $1, Record* $3) {
    char* tempString = malloc(strlen($1) - 1);
...
void ID_toInput(Record** $$, ID_t $1, Record* $3) {
    ...
    if (a == CONSTANT) {
        printf("ERROR Line %d: the variable %s is constant\n",
               yylineno, $1);
        exit(1);
    }
    if (a == STAT) {
        printf("ERROR Line %d: the variable %s is not mutable\n",
               yylineno, $1);
        exit(1);
    }
    // TODO: O que aconteceria se getvartype é null?
