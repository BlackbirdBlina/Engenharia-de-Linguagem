...
type checkTypeCompat(char* type1, char* type2, compatDirection direction) {
    // Returns null
    ...
    if (isArrayType1 != NULL && isArrayType2 != NULL) {
        ...
        if (arraySizeType1 == arraySizeType2) {
            return checkTypeCompat(isArrayType1, isArrayType2, direction);
        } else {
    ...
    if (direction == RIGHT_LEFT || direction == BOTH) {
